...
class Botton():
    def __init__(self, x, y, imagen: pygame.surface.Surface, escala):
...
class NaveEnemiga(pygame.sprite.Sprite):
    ...
    def movimiento(self, surface: pygame.surface.Surface, enemigos_sprites:pygame.sprite.Group, 
...
class NaveEnemiga_2(pygame.sprite.Sprite):

    def __init__(self, x, y, escala, velocidad_disparo, cadencia):
        super().__init__()
        #establecimiento del objeto y su imagen
        self.image = pygame.image.load(f"imagenes\\NAVE_ENEMIGO_4.png")# imagen de la nave
        #ajuste de tamaño y definicion del hitbox
        ancho = self.image.get_width()
        alto = self.image.get_height()
        self.image = pygame.transform.scale(self.image, (int(ancho * escala), int(alto * escala)))
        ...
        self.velocidad_y = 3
        #disparo
        self.ultimo_disparo2 = pygame.time.get_ticks()
        self.cadencia = cadencia
        self.lista_disparos2 = []
        self.velocidad_disparo = velocidad_disparo

    def movimiento(self, surface: pygame.surface.Surface, enemigos_sprites2:pygame.sprite.Group, 
                   disparo_enemigo_sprites: pygame.sprite.Group):
        disparo_enemigo_sprites = disparo_enemigo_sprites
        ...
        for x in enemigos_sprites2:
            tiempo_enemigo2 = pygame.time.get_ticks()
            if tiempo_enemigo2 - x.ultimo_disparo2 > x.cadencia:
                x_1, y_1 = x.rect.center        

                proyectil = x.disparo(x_1, y_1)

                proyectil2 = x.disparo(x_1 - 25, y_1)
                
                x.lista_disparos2.append(proyectil)

                x.ultimo_disparo2 = tiempo_enemigo2

                disparo_enemigo_sprites.add(proyectil)
                disparo_enemigo_sprites.add(proyectil2)
    ...
    def disparo(self, x, y):
        ...
        self.lista_disparos2.append(self.proyectil_enemigo)
