    ...
    while correr_juego:
        ...
        if not juego:
            ...
            if niveles and not opciones:
                ...
                dibujado_de_texto(PANTALLA, "NOMBRE    TIEMPO    SCORE", fuente_puntos, BLANCO, NEGRO, 100, 150, True)
                with sqlite3.connect("base.db") as conexion:
                    try:
                        cursor2 = conexion.execute("SELECT * FROM jugadores")
                        for fila in cursor2:
                            dibujado_de_texto(PANTALLA, f"{fila[1]}             {fila[2]}                   {fila[3]}", fuente_puntos, BLANCO, NEGRO, 100, 170, True)
                        # for fila2 in cursor2:
                        #     if fila2[0] >= 1:
                        #         dibujado_de_texto(PANTALLA, f"{fila2[1]}            {fila2[2]}                   {fila2[3]}", fuente_puntos, BLANCO, NEGRO, 100, 200, True)
        ...
        if juego:
            ...
            print(score_jugador, cronometro)
...
def Galaxy(PANTALLA: pygame.surface.Surface, fondo: pygame.surface.Surface, nivel: str, cadencia: int, jugador_sprites: pygame.sprite.Group, 
    ...
    if player__.vida > 0:
        ...
        dibujado_de_texto(PANTALLA, f"  SCORE   {player__.score}   ", fuente_game_over, BLANCO, NEGRO, 150, 100 )
        dibujado_de_texto(PANTALLA, f"  TIEMPO    {tiempo}", fuente_game_over, BLANCO, NEGRO, 150, 200)
